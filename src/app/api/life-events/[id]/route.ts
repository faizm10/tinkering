import { completeLifeEvent, deleteLifeEvent, updateLifeEvent } from "@/server/services/manual-service";
...
export async function DELETE(_request: Request, context: { params: Promise<unknown> }) {
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Could not delete life event." }, { status: 400 });
  }
}
